function [x, error, its, flag] = gmres_dq_vpa( A, x, b, L, U, restrt, max_it, tol)
...
%   which is done in quad precision using vpa
%
...
rtmp = b-A*x;
r = vpa(double(L),32)\vpa(double(rtmp),32);
r = vpa(double(U),32)\r;
r = double(r);
...
for iter = 1:max_it,                              % begin iteration
    ...
    r = vpa(double(U),32)\(vpa(double(L),32)\vpa(double(rtmp),32));
    r = double(r);
    ...
    for i = 1:m,                     % construct orthonormal basis via GS
        ...
        vcur = vpa(double(U),32)\(vpa(double(L),32)\(vpa(double(A),32)*vpa(double(vcur),32)));
    ...
    rtmp = b-A*x;
     ...
     r = vpa(double(U),32)\(vpa(double(L),32)\vpa(double(rtmp),32));           % compute residual
     r = double(r);
